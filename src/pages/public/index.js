...
document.addEventListener("htmx:after:request", (event) => {
  ...
  if (target instanceof HTMLFormElement && target.matches("[data-reset-after-success]")) {
    target.reset();
  }
});
...
document.addEventListener("htmx:before:cleanup", (event) => {
  ...
  if (cleanupRoot instanceof HTMLInputElement && cleanupRoot.matches("[data-image-input]")) {
    revokePreviewUrl(cleanupRoot);
  }
  for (const input of cleanupRoot.querySelectorAll("[data-image-input]")) {
    revokePreviewUrl(input);
  }
});
function htmxRequestContext(event) {
...
function showSelectedImage(input) {
  ...
  const removeControl = input.dataset.removeImage ? document.getElementById(input.dataset.removeImage) : null;
...
function toggleImageRemoval(removeControl) {
  ...
  if (!removeControl.checked)
    return;
  const row = removeControl.closest("tr");
  const imageInput = row?.querySelector("[data-image-input]");
  if (imageInput) {
    imageInput.value = "";
...
function resetPreview(input, showCurrent = true) {
  revokePreviewUrl(input);
  const preview = imageById(input.dataset.imagePreview);
  preview?.removeAttribute("src");
  preview?.classList.add("hidden");
